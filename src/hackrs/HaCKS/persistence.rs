...
impl HaCKS {
    ...
    where
        F: Fn(&HashMap<String, Rc<RefCell<dyn HaCK>>>) -> HashMap<String, serde_json::Value>,
    {
        ...
        F: Fn() -> Vec<Rc<RefCell<dyn HaCK>>>,
    {
        ...
        self.hacs.clear();
        self.loaded_libs.borrow_mut().clear();
        ...
        F: Fn() -> Vec<Rc<RefCell<dyn HaCK>>>,
    {
        ...
        // Find key by TypeId
        let key_to_remove = self.hacs.iter().find_map(|(k, v)| {
        ...
        if let Some(key) = key_to_remove {
            if self.hacs.remove(&key).is_some() {
                for module in create_modules_fn() {
