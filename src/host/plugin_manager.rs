...
pub fn render_plugin_manager_inner(
    ...
    if !hacks.hacs.is_empty() {
        for (i, (name, hack)) in hacks.hacs.iter().enumerate() {
            ...
            if load_type != metadata::HaCKLoadType::Plugin {
                internal_count += 1;
    ...
    // Changed from env!("CARGO_MANIFEST_DIR") to runtime relative path "./plugins"
    // This assumes the runner is executed from a directory containing "plugins"
