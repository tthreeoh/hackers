...
impl<T: Clone + PartialEq> Default for StateAggregate<T> {
    fn default() -> Self {
        Self {
...
impl<T: Clone + PartialEq> StateAggregate<T> {
    fn update(&mut self, new_duration: Duration, state: T) {
...
/// Generic state tracking system with customizable formatting

#[derive(Clone, Debug)]
pub struct StateTracker<T: Clone + PartialEq, F: StateFormatter<T> = ()> {
    pub enabled: bool,
    pub dock: bool,
    pub show: bool,
...
impl<T: Clone + PartialEq, F: StateFormatter<T>> Default for StateTracker<T, F> 
where F: Default 
{
    fn default() -> Self {
        Self {
...
impl<T: Clone + PartialEq, F: StateFormatter<T>> StateTracker<T, F> {
    /// Records a state transition, updating durations for the previous state
        ...
        if let (Some(start), Some(state)) = (self.current_state_start, self.current_state.clone()) {
            ...
            match aggregate {
                ...
                None => {
                    ...
                    let mut new_agg = StateAggregate::default();
                    new_agg.update(elapsed, state);
    ...
    pub fn get_active_session_duration(&self) -> Duration {
        // Return total active time plus current active segment if any
...
//         let mut tracker = StateTracker::<TestState>::default();
//         // Simulate multiple state transitions
//         tracker.state_change(TestState::Init);
//         sleep(Duration::from_millis(10));
//         tracker.state_change(TestState::Processing);
//         sleep(Duration::from_millis(20));
//         tracker.state_change(TestState::Complete);
//         sleep(Duration::from_millis(30));
//         // Retrieve and validate statistics
//         let stats = tracker.get_state_statistics();
//         assert_eq!(stats.len(), 3);
//         for (state, stat) in stats {
//             match state {
//                 TestState::Init => {
//                     assert!(stat.total_duration.as_millis() >= 10);
//                     assert_eq!(stat.occurrences, 1);
//                 },
//                 TestState::Processing => {
//                     assert!(stat.total_duration.as_millis() >= 20);
//                     assert_eq!(stat.occurrences, 1);
//                 },
//                 TestState::Complete => {
//                     assert!(stat.total_duration.as_millis() >= 30);
//                     assert_eq!(stat.occurrences, 1);
//                 }
//             }
//         }
