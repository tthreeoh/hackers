    ...
    pub fn update(
        ...
        match self.control_mode {
            ControlMode::TopDown => {
                ...
                if ui.is_key_down(hackers::gui::Key::W) {
                    delta_y -= speed;
                    input_active = true;
                    self.facing_direction = Direction::Up;
                }
                if ui.is_key_down(hackers::gui::Key::S) {
                    delta_y += speed;
                    input_active = true;
                    self.facing_direction = Direction::Down;
                }
                if ui.is_key_down(hackers::gui::Key::A) {
                    delta_x -= speed;
                    input_active = true;
                    self.facing_direction = Direction::Left;
                }
                if ui.is_key_down(hackers::gui::Key::D) {
                    delta_x += speed;
                    input_active = true;
            ...
            ControlMode::SideScroll => {
                ...
                match self.jump_style {
                    JumpStyle::Normal => {
                        if ui.is_key_down(hackers::gui::Key::Space) {
                            if self.is_grounded {
                        ...
                        if !ui.is_key_down(hackers::gui::Key::Space) && self.velocity[1] < -0.1 {
                    ...
                    JumpStyle::Charge => {
                        if ui.is_key_down(hackers::gui::Key::Space) {
                            if self.is_grounded {
                                ...
                                // Immediate wall jump for charge mode? Or charge wall jump?
                                // Let's do immediate wall jump for fluidity for now, or maybe same logic?
                                // User asked for "jump on release", implying ground.
                                // Let's keep wall jump instantaneous for now as it's a reaction mechanic.
                                if self.on_wall_left {
                                    self.velocity[0] = self.wall_jump_force[0];
                                    self.velocity[1] = -self.wall_jump_force[1];
                                } else if self.on_wall_right {
                ...
                // Run Logic (Shift)
                let is_running = ui.is_key_down(hackers::gui::Key::LeftShift)
                    || ui.is_key_down(hackers::gui::Key::RightShift);
                let current_move_speed = if is_running {
                    self.max_run_speed
                } else {
                    self.speed
                };
                ...
                // Horizontal Movement
                if ui.is_key_down(hackers::gui::Key::LeftArrow)
                    || ui.is_key_down(hackers::gui::Key::A)
                {
                    self.velocity[0] = -current_move_speed;
                    self.facing_direction = Direction::Left;
                    input_active = true;
                    if self.is_grounded {
                ...
                } else if ui.is_key_down(hackers::gui::Key::RightArrow)
                    || ui.is_key_down(hackers::gui::Key::D)
                {
                    self.velocity[0] = current_move_speed;
                    self.facing_direction = Direction::Right;
                    input_active = true;
